...
structure Parser =  struct
  ...
  fun stringOfToken T_LET = "T_LET"
    ...
    | stringOfToken T_AND = "T_AND"
    | stringOfToken T_OR = "T_OR"
    | stringOfToken T_NOT = "T_NOT"
    | stringOfToken T_COND = "T_COND"
    | stringOfToken T_LIST = "T_LIST"
    | stringOfToken T_CASE = "T_CASE"
  ...
  fun produceSymbol "let" = SOME (T_LET)
    ...
    | produceSymbol "nil" = SOME (T_NIL)
      (* FOR TRANSFORMATIONS *)
    | produceSymbol "and" = SOME (T_AND)
    | produceSymbol "or" = SOME (T_OR)
    | produceSymbol "not" = SOME (T_NOT)
    | produceSymbol "list" = SOME (T_LIST)
    | produceSymbol "cond" = SOME (T_COND)
    | produceSymbol "case" = SOME (T_CASE)
   ...
   *                 T_SYM expr_list
   *                   (* FOR TRANSFORMATIONS *)
   *                 T_AND expr expr T_RPAREN
   *                 T_OR expr expr T_RPAREN
   *                 T_NOT expr T_RPAREN
   *                 T_LIST expr_list
   *                 T_COND cond_list
   *                 T_CASE expr expr T_LPAREN T_LPAREN T_SYM T_SYM T_RPAREN 
   *                                       expr T_RPAREN T_RPAREN
   *
   *   expr_list ::= T_RPAREN
   *                 expr expr_list
   * 
   *   symbol_list ::= T_RPAREN
   *                   T_SYM symbol_list
   *
   *   cond_list ::= T_RPAREN
   *                 T_LPAREN expr expr R_RPAREN cond_list
   *)
  ...
  fun expect_AND (T_AND::ts) = ts
    | expect_AND _ = parseError "expect_AND"

  fun expect_OR (T_OR::ts) = ts
    | expect_OR _ = parseError "expect_OR"

  fun expect_NOT (T_NOT::ts) = ts
    | expect_NOT _ = parseError "expect_NOT"

  fun expect_LIST (T_LIST::ts) = ts
    | expect_LIST _ = parseError "expect_LIST"
		       
  fun expect_COND (T_COND::ts) = ts
    | expect_COND _ = parseError "expect_COND"
		       
  fun expect_CASE (T_CASE::ts) = ts
    | expect_CASE _ = parseError "expect_CASE"
  ...
  and parse_expr_seq [] = parseError "unexpected end of token sequence"
    ...
    | parse_expr_seq (T_AND::ts) = let       (* FOR TRANSFORMATIONS *) 
	  val (e1,ts) = parse_expr ts
	  val (e2,ts) = parse_expr ts
	  val ts = expect_RPAREN ts
      in
	  (I.EIf (e1,e2,I.EVal (I.VBool false)),ts)
      end
    | parse_expr_seq (T_OR::ts) = let
	  val (e1,ts) = parse_expr ts
	  val (e2,ts) = parse_expr ts
	  val ts = expect_RPAREN ts
      in
	  (I.EIf (e1,I.EVal (I.VBool true),e2),ts)
      end
    | parse_expr_seq (T_NOT::ts) = let
	  val (e1,ts) = parse_expr ts
	  val ts = expect_RPAREN ts
      in
	  (I.EIf (e1,I.EVal (I.VBool false), I.EVal (I.VBool true)),ts)
      end
    | parse_expr_seq (T_LIST::ts) = let
	  val (es,ts) = parse_expr_list ts
	  fun convert [] = I.EVal (I.VList [])
	    | convert (e::es) = I.ECons (e,convert es)
      in
	  (convert es,ts)
      end
    | parse_expr_seq (T_COND::ts) = let
	val (cps,ts) = parse_cond_list ts
	fun convert [] = I.EVal (I.VBool false)
	  | convert ((e1,e2)::cps) = I.EIf (e1,e2,convert cps)
      in
	  (convert cps,ts)
      end
    | parse_expr_seq (T_CASE::ts) = let
	val (e_list, ts) = parse_expr ts
	val (e_nil, ts) = parse_expr ts
	val ts = expect_LPAREN ts
	val ts = expect_LPAREN ts
	val (s_head,ts) = expect_SYM ts
	val (s_tail,ts) = expect_SYM ts
	val ts = expect_RPAREN ts
	val (e_cons, ts) = parse_expr ts
	val ts = expect_RPAREN ts
	val ts = expect_RPAREN ts
      in
	(I.EIf (I.EEq (e_list,I.EVal (I.VList [])),
		e_nil,
		I.ELet (s_head, I.EHead (e_list),
			I.ELet (s_tail, I.ETail (e_list),
				e_cons))), ts)
      end
    | parse_expr_seq _ = parseError "unexpected token"


  and parse_expr_list (T_RPAREN::ts) = ([],ts)
    | parse_expr_list ts = let
  ...
  and parse_cond_list (T_RPAREN::ts) = ([],ts)
    | parse_cond_list (T_LPAREN::ts) = let
	val (e1,ts) = parse_expr ts
	val (e2,ts) = parse_expr ts
	val ts = expect_RPAREN ts
	val (cps,ts) = parse_cond_list ts
    ...
    | parse_cond_list _ = parseError "unexpected token"
