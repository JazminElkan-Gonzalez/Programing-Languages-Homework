...
fun subst (EVal value) id e = EVal value
  ...
  | subst (ECall (n,f)) id e = ECall (n,subst f id e)
...
fun eval _ (EVal v) = v
  ...
  | eval fenv (ECall (name,expr)) = 
                evalCall fenv (lookup name fenv) (evalList fenv expr)

and evalCall fenv (FDef (param,body)) v = 
       eval fenv (substAll body param v)
...
fun substAll e [] [] = e
  | substAll (sudst e id (EVal v)) ids v 
  | sudstAll _ _ _ = raise EvalError "substAll"
